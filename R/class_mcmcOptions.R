...
toString.mcmcOptions<-function(x,...){
  value=list(x$result.fname,x$nAdapt,x$nCycles,
             ...
             0, # Mode for setting the initial Std of the jump distribution - 0=auto
             '****    DEFINITION OF INITIAL JUMP STD   ****', # just a cosmetics line
             x$multFactor,
             -9999,-9999 # manual mode stuff, disabled
             )
  comment=c(
    ...
    'multFactor, multiplicative factor to set initial jump standard deviations to multFactor*|initValue|',
    'manual mode stuff, disabled',
    'manual mode stuff, disabled'
  )
...
new_mcmcOptions<-function(fname,result.fname,nAdapt,nCycles,
                   minMoveRate,maxMoveRate,downMult,upMult,multFactor){
