...
# Also for VAR parameters, the prior distribution for b/k's has to be 'Gaussian',
...
runSPD <-function(workspace,controlMatrix,pars,
  ...
  BaM(workspace=workspace,mod=M,data=D,remnant=list(remnant),preClean=FALSE)
}

workspace=file.path(getwd(),'BaM_workspace')
runSPD(workspace=workspace,controlMatrix=controlMatrix,pars=pars,
